...
contract = web3.eth.contract(address=Web3.to_checksum_address(CONTRACT_ADDRESS), abi=BEP20_ABI)
...
async def procesar_reclamo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if balance < MIN_TOKENS_TO_CLAIM:
        await update.message.reply_text(f"❌ Tu wallet solo tiene {balance:.2f} Vyrax. Se requieren al menos 700.")
        return
    ...
    await update.message.reply_text(f"✅ {cantidad} Vyrax enviados a {wallet}.\n🧾 TX: https://bscscan.com/tx/{tx_hash}")
